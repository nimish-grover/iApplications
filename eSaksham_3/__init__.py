...
from eSaksham_3.app import create_app
...
def not_found_error(error):
    ...
    if not request.path.startswith('/scorm') and 'favicon' not in request.path:
        ...
        modified_path = '/esaksham_3/scorm' + request.path
        # Redirect to the modified path
        return redirect(modified_path)
    elif request.path.startswith('/scorm'):
        modified_path = '/esaksham_3' + request.path
        return redirect(modified_path)
    elif not request.path.startswith('/esaksham_3/scorm'):
        modified_path = '/esaksham_3/scorm' + request.path
        return redirect(modified_path)
