...
class Water_bodies_ap(db.Model):
    __tablename__ = 'water_bodies_ap'
    ...
    def delete_from_db(_code):
        participant = Water_bodies_ap.query.filter_by(code=_code).first()
    ...
    def update_db(data,_code):
        user = Water_bodies_ap.query.filter_by(code=_code).update(data)
        db.session.commit()
    ...
    #         func.concat('[', func.to_char(func.sum(Water_bodies_ap.storage_capacity), 'FM999999999.00'), ',',
    #         func.count(Water_bodies_ap.wb_type_id), ',',
    #         func.to_char(func.sum(Water_bodies_ap.water_spread_area), 'FM999999999'), ',"',
    #         District.name, '"]'),
    #         func.to_char(func.sum(Water_bodies_ap.storage_capacity), 'FM999999999.00').label('storage_capacity'),
    #         func.count(Water_bodies_ap.wb_type_id).label('count'),
    #         func.to_char(func.sum(Water_bodies_ap.water_spread_area), 'FM999999999.00').label('spread_area'),
    #         District.name).\
    #         join(Village, Village.code == Water_bodies_ap.village_code).\
    #         join(District, District.id == Village.district_id).\
    #         join(State, State.code == District.state_id).\
    #         join(Block, Block.district_id == Water_bodies_ap.district_code).\
    #         group_by(District.id, District.name).\
    #         order_by(func.count(Water_bodies_ap.wb_type_id).desc())
    ...
    #     func.count(Water_bodies_ap.wb_type_id).label('count'),
    #     WB_master.name).\
    #     join(WB_master, WB_master.code == Water_bodies_ap.wb_type_id).\
    #     join(District, District.code == Water_bodies_ap.district_code).\
    #     join(State, State.code == District.state_id).\
    #     join(Village, Village.code == Water_bodies_ap.village_code).\
    #     join(Block, Block.district_id == Water_bodies_ap.district_code).\
