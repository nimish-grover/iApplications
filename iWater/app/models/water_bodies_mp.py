...
class Water_bodies_mp(db.Model):
    __tablename__ = 'water_bodies_mp'
    ...
    @classmethod
    def get_existing_data(cls, json_data,_wb_type_id):
        # query = db.session.query(filter(cls.livestock_id ==_livestock_id))
        query=cls.query.filter_by(wb_type_id=_wb_type_id)
    ...
    def delete_from_db(_code):
        participant = Water_bodies_mp.query.filter_by(code=_code).first()
    ...
    def update_db(data,_code):
        user = Water_bodies_mp.query.filter_by(code=_code).update(data)
        db.session.commit()
    ...
    #         func.concat('[', func.to_char(func.sum(Water_bodies_mp.storage_capacity), 'FM999999999.00'), ',',
    #         func.count(Water_bodies_mp.wb_type_id), ',',
    #         func.to_char(func.sum(Water_bodies_mp.water_spread_area), 'FM999999999'), ',"',
    #         District.name, '"]'),
    #         func.to_char(func.sum(Water_bodies_mp.storage_capacity), 'FM999999999.00').label('storage_capacity'),
    #         func.count(Water_bodies_mp.wb_type_id).label('count'),
    #         func.to_char(func.sum(Water_bodies_mp.water_spread_area), 'FM999999999.00').label('spread_area'),
    #         District.name).\
    #         join(Village, Village.code == Water_bodies_mp.village_code).\
    #         join(District, District.id == Village.district_id).\
    #         join(State, State.code == District.state_id).\
    #         join(Block, Block.district_id == Water_bodies_mp.district_code).\
    #         group_by(District.id, District.name).\
    #         order_by(func.count(Water_bodies_mp.wb_type_id).desc())
    ...
    #     func.count(Water_bodies_mp.wb_type_id).label('count'),
    #     WB_master.name).\
    #     join(WB_master, WB_master.code == Water_bodies_mp.wb_type_id).\
    #     join(District, District.code == Water_bodies_mp.district_code).\
    #     join(State, State.code == District.state_id).\
    #     join(Village, Village.code == Water_bodies_mp.village_code).\
    #     join(Block, Block.district_id == Water_bodies_mp.district_code).\
    ...
    def get_waterbodies(cls, json_data):
        query = db.session.query(
            cls.water_spread_area.label('area'),
            case(
                    (cls.water_spread_area < 10, 'small'),
                    (cls.water_spread_area > 100, 'large'),
                else_='medium'
            ).label('waterbody')
        ).join(Village, Village.id == cls.village_code)
